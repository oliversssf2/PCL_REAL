...
ImGui::Begin(const char *name, bool *p_open, const ImVec2 &size_on_first_use, float bg_alpha, ImGuiWindowFlags flags) {
	...
	if (first_begin_of_the_frame) {
		...
		if (window->ScrollTarget.y < FLT_MAX) {
			...
			window->Scroll.y = window->ScrollTarget.y - ((1.0f - center_ratio) * window->TitleBarHeight()) -
		...
		if (window->Collapsed) {
			...
			if (bg_color.w > 0.0f)
				window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()),
				                                ...
				                                window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? 15 : 4 | 8);
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
				window->DrawList->AddRectFilled(title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetColorU32(
						...
						? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg), window_rounding, 1 | 2);
			...
			if (flags & ImGuiWindowFlags_MenuBar) {
				...
				if (flags & ImGuiWindowFlags_ShowBorders)
					window->DrawList->AddLine(menu_bar_rect.GetBL() - ImVec2(0, 0),
					                          menu_bar_rect.GetBR() - ImVec2(0, 0), GetColorU32(ImGuiCol_Border));
			}
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
	...
	if (horizontal)
		...
		window_rounding_corners =
				(((window->Flags & ImGuiWindowFlags_NoTitleBar) && !(window->Flags & ImGuiWindowFlags_MenuBar)) ? 2
				                                                                                                : 0) |
				(other_scrollbar ? 0 : 4);
...
void ImGui::SetScrollY(float scroll_y) {
	...
	window->ScrollTarget.y =
			scroll_y + window->TitleBarHeight(); // title bar height canceled out when using ScrollTargetRelY
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
	...
	if (display_frame) {
		...
		if (flags & ImGuiTreeNodeFlags_Bullet)
			...
			RenderCollapseTriangle(bb.Min + ImVec2(padding.x, padding.y + g.FontSize * 0.15f + text_base_offset_y),
			                       is_open, 0.70f, false);
		if (g.LogEnabled)
...
bool ImGui::SliderBehavior(const ImRect &frame_bb, ImGuiID id, float *v, float v_min, float v_max, float power,
                           int decimal_precision, ImGuiSliderFlags flags, bool render_bg) {
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = GetCurrentWindow();
	const ImGuiStyle &style = g.Style;
	...
	if (render_bg) {
		...
		RenderFrame(fill_br.Min, fill_br.Max, ImGui::ColorConvertFloat4ToU32({0, 112.f / 255, 197.f / 255, 1}), false,
		            10.0f);
		window->DrawList->AddCircleFilled(graber_size, radius, GetColorU32(
				g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 16);
	} else {
...
bool ImGui::VSliderFloat(const char *label, const ImVec2 &size, float *v, float v_min, float v_max,
                         const char *display_format, float power, bool render_bg) {
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision,
	                                    ImGuiSliderFlags_Vertical, render_bg);
...
bool
ImGui::SliderIntWithSteps(const char *label, int *v, int v_min, int v_max, int v_step, const char *display_format) {
	if (!display_format)
		display_format = "%d";
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay) {
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x,
		                  ...
		                  overlay, NULL, &overlay_size, ImGuiAlign_Left | ImGuiAlign_VCenter, &bb.Min, &bb.Max);
}
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags,
	...
	if (g.ActiveId == id) {
		...
		const float mouse_y = (is_multiline ? (g.IO.MousePos.y - draw_window->DC.CursorPos.y - style.FramePadding.y) : (
				g.FontSize * 0.5f));
		...
		const bool is_shortcutkey_only = (io.ShortcutsUseSuperKey ? (is_super_down && !is_alt_down && !is_shift_down &&
		                                                             !is_ctrl_down) : (is_ctrl_down && !is_alt_down &&
		                                                                               !is_shift_down &&
		                                                                               !is_super_down));
		...
		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
			edit_state.OnKeyPressed(
					is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT | k_mask : STB_TEXTEDIT_K_LEFT | k_mask);
		}
		else if (IsKeyPressedMap(ImGuiKey_RightArrow)) {
			edit_state.OnKeyPressed(
					is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT | k_mask : STB_TEXTEDIT_K_RIGHT | k_mask);
		}
		...
		} else if ((flags & ImGuiInputTextFlags_AllowTabInput) && IsKeyPressedMap(ImGuiKey_Tab) && !is_ctrl_down &&
	...
	if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID("#SCROLLY"))) {
		...
		draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - render_scroll, GetColorU32(ImGuiCol_Text), buf,
		...
		if (is_multiline)
			text_size = ImVec2(size.x,
			                   InputTextCalcTextLenAndLineCount(buf, &buf_end) * g.FontSize); // We don't need width
		draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos, GetColorU32(ImGuiCol_Text), buf, buf_end, 0.0f,
...
ImGui::InputTextMultiline(const char *label, char *buf, size_t buf_size, const ImVec2 &size, ImGuiInputTextFlags flags,
                          ImGuiTextEditCallback callback, void *user_data) {
	bool ret = InputTextEx(label, buf, (int) buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback,
	                       user_data);
	return ret;
}
...
bool ImGui::Combo(const char *label, int *current_item, const char *items_separated_by_zeros, int height_in_items,
	...
	bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void *) items_separated_by_zeros,
	                           items_count, height_in_items, show_arrow_down);
	return value_changed;
}

// Combo box function.
bool ImGui::Combo(const char *label, int *current_item, bool (*items_getter)(void *, int, const char **), void *data,
                  int items_count, int height_in_items, bool show_arrow_down) {
	ImGuiWindow *window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::ColorButton(const ImVec4 &col, bool small_height, bool outline_border) {
	...
	if (hovered)
		SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X", col.x, col.y, col.z, col.w,
		           IM_F32_TO_INT8(col.x), IM_F32_TO_INT8(col.y), IM_F32_TO_INT8(col.z), IM_F32_TO_INT8(col.z));
...
void ImGui::Separator() {
	...
	ItemSize(ImVec2(0.0f,
	                0.0f)); // NB: we don't provide our width so that it doesn't get feed back into AutoFit   // FIXME: Height should be 1.0f not 0.0f ?
	if (!ItemAdd(bb, NULL)) {
		if (window->DC.ColumnsCount > 1)
			PushColumnClipRect();
		return;
...
void ImGui::Columns(int columns_count, const char *id, bool border) {
	...
	window->DC.ColumnsMaxX = content_region_width -
	                         window->Scroll.x /*- ((window->Flags & ImGuiWindowFlags_NoScrollbar) ? 0 : g.Style.ScrollbarSize)*/;// - window->WindowPadding().x;
	window->DC.ColumnsStartPosY = window->DC.CursorPos.y;
	...
	if (window->DC.ColumnsCount != 1) {
		...
		for (int column_index = 0; column_index < columns_count + 1; column_index++) {
			...
			const float t = window->DC.StateStorage->GetFloat(column_id,
			                                                  default_t);      // Cheaply store our floating point value inside the integer (could store an union into the map?)
			window->DC.ColumnsData[column_index].OffsetNorm = t;
		}

...
namespace ImGui {
	...
	// Widgets
	IMGUI_API void
	Icon(const char *icon);                                                 // Librealsense addition to allow type safe calling like: ImGui::Icon(textual_icon(u8"\uf118"))
	...
	IMGUI_API bool
	Combo(const char *label, int *current_item, const char *items_separated_by_zeros, int height_in_items = -1,
	      bool show_arrow_down = true);      // separate items with \0, end item-list with \0\0
	IMGUI_API bool
	Combo(const char *label, int *current_item, bool (*items_getter)(void *data, int idx, const char **out_text),
	      void *data, int items_count, int height_in_items = -1, bool show_arrow_down = true);
	...
	SliderFloat(const char *label, float *v, float v_min, float v_max, const char *display_format = "%.3f",
	            float power = 1.0f,
	            bool render_bg = false);     // adjust display_format to decorate the value with a prefix or a suffix. Use power!=1.0 for logarithmic sliders
	IMGUI_API bool
	...
	IMGUI_API bool VSliderFloat(const char *label, const ImVec2 &size, float *v, float v_min, float v_max,
	                            const char *display_format = "%.3f", float power = 1.0f, bool render_bg = false);
	...
	IMGUI_API float
	GetTreeNodeToLabelSpacing();                                            // horizontal distance preceeding label when using TreeNode*() or Bullet() == (g.FontSize + style.FramePadding.x*2) for a regular unframed TreeNode
	...
	IMGUI_API bool IsRectVisible(
			const ImVec2 &size);                                  // test if rectangle of given size starting from cursor pos is visible (not clipped). to perform coarse clipping on user's side (as an optimization)
...
enum ImGuiSetCond_ {
	...
	1 << 3  // Only set the variable if the window is appearing after being inactive (or the first time)
};
...
struct ImDrawList {
	...
	IMGUI_API void
	AddRect(const ImVec2 &a, const ImVec2 &b, ImU32 col, float rounding = 0.0f, int rounding_corners = 0x0F,
	...
	IMGUI_API void
	PathRect(const ImVec2 &rect_min, const ImVec2 &rect_max, float rounding = 0.0f, int rounding_corners = 0x0F);

...
void ImGui_ImplGlfw_NewFrame(float scale_factor) {
	...
	if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED)) {
		...
		io.MousePos = ImVec2((float) mouse_x / scale_factor, (float) mouse_y /
		                                                     scale_factor);   // Mouse position in screen coordinates (set to -1,-1 if no mouse / on another screen, etc.)
	} else {

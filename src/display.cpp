...
pcl::visualization::PCLVisualizer::Ptr simpleVis(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, int pointSize) {
	// --------------------------------------------
	...
	viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, pointSize, "sample cloud");
...
main(int argc, char **argv) {
	...
	pcl::PointCloud<pcl::PointNormal>::Ptr pc_n(new pcl::PointCloud<pcl::PointNormal>);


//	if(normals)
//	{
//		pcl::io::loadPCDFile<pcl::PointNormal> (fileName, *pc_n) ; //* load the file
//	}
	if (simple) {
		if (pcl::io::loadPCDFile<pcl::PointXYZ>(fileName, *pc) == -1) //* load the file
		{
